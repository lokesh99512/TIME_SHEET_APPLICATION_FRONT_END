...
import { ADD_OBJECT_INSTANT_SEARCH, BLANK_INSTANT_SEARCH, GET_INSTANT_AIR_SEARCH_RESULT_DETAILS, QUOTATION_RESULT_SELECTED_BLANK, REMOVE_OBJECT_INSTANT_SEARCH } from "../../store/InstantRate/actionType";
                        ...
                        <div className="create_sales_wrapper_inner">
                            <div className="d-flex justify-content-end main_tab_wrap">
                                <button type="button" className={`btn d-flex align-items-center ${mainactiveTab === "ocean_freight" ? "active" : ""}`}
                                    onClick={() => { setSearchResult(false); blankValueHandler(); setMainactiveTab("ocean_freight"); toggleTab("FCL"); dispatch({ type: REMOVE_OBJECT_INSTANT_SEARCH, payload: 'shipment_details' }) }}>
                                ...
                                <button type="button" className={`btn d-flex align-items-center ms-3 ${mainactiveTab === "air_freight" ? "active" : ""}`}
                                    onClick={() => { setSearchResult(false); blankValueHandler(); setMainactiveTab("air_freight"); toggleTab("dom_air"); dispatch({ type: ADD_OBJECT_INSTANT_SEARCH, payload: 'shipment_details' }); }}>
                            ...
                            <TabContent activeTab={activeTab}>
                                <TabPane tabId={activeTab}>
                                    <SearchForm activeTab={activeTab} searchQuoteHandler={showSearchResultHandler} mainactiveTab={mainactiveTab} />
                                </TabPane>
                            </TabContent>
                            {/* --------------------------tabs------------------------------- */}

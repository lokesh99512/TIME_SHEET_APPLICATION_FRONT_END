...
function* getCompanyCityDetails() {
    try {
        ...
        yield put({ type: GET_PARTIES_COMPANY_CITY_DATA_SUCCESS, payload: response });
    } catch (error) {
        // showErrorToast(error?.message);
...
function* getCompanyStateDetails({ payload: { cityId } }) {
    try {
        ...
        // console.log(response, "--respnse")
        yield put({ type: GET_PARTIES_COMPANY_STATE_DATA_SUCCESS, payload: response });
    } catch (error) {
        // showErrorToast(error?.message);
...
function* getCompanyCountryDetails({ payload: { cityId } }) {
    try {
        ...
        // console.log(response, "--respnse")
        yield put({ type: GET_PARTIES_COMPANY_COUNTRY_DATA_SUCCESS, payload: response });
    } catch (error) {
        // showErrorToast(error?.message);
        console.log(error, "saga state api error")
    }
...
function* getCompanyPincodeDetails({ payload: { cityId } }) {
    try {
        ...
        yield put({ type: GET_PARTIES_COMPANY_PINCODE_DATA_SUCCESS, payload: response });
    } catch (error) {
        // showErrorToast(error?.message);
        console.log(error, "saga state api error")
    }
...
function* getAllPartiesEmployeeDetails(){
    try {
        ...
        console.log(response, "reponse into getAllPartiesCompanySettings")
        yield put({ type: GET_PARTIES_CUSTOMER_EMPLOYEE_DETAILS_SUCCESS, payload: response});
    } catch (error) {
        console.log(error, "saga getAllCompanySettings api error");
    }
}
...
export function* watchGetPartiesCustomersData(){
    ...
    yield takeLatest(GET_PARTIES_CUSTOMERS_DETAILS, getPartiesCustomerDetailsData)
